...
async function loadUsers() {
  ...
  tbody.innerHTML = `<tr><td colspan="8" class="muted">Loading users...</td></tr>`;
  ...
  try {
    ...
    tbody.innerHTML = users.map(user => {
      ...
      return `
        <tr data-user-id="${escapeHtml(user.id)}">
          ...
          <td>
            <span class="badge ${user.premium ? "premium" : "free"}">${user.premium ? "PREMIUM" : "FREE"}</span>
            <small class="muted">${premiumTime}</small>
          </td>
          <td>
            <span class="badge ${key?.status === "active" ? "active" : "inactive"}">${keyStatus}</span>
            <small class="muted">${keyTime}</small>
          </td>
    ...
    tbody.innerHTML = `<tr><td colspan="8" class="error-cell">${escapeHtml(error.message)}</td></tr>`;
    if (/Unauthorized|Session expired|Forbidden/i.test(error.message)) {
...
async function adjustKey(userId, hours) {
  try {
    await api("/api/admin/free-key", {
      method: "POST",
      body: JSON.stringify({
        user_id: Number(userId),
        hours_delta: Number(hours)
      })
    });
...
function setupEvents() {
  ...
  $("#adminUsersBody")?.addEventListener("click", event => {
    ...
    if (action === "add-key") {
      adjustKey(id, Number(button.dataset.hours));
    }

    if (action === "premium") {
      const select = document.querySelector(`select[data-user-id="${CSS.escape(id)}"]`);
