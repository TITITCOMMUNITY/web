  ...
  try {
    ...
    const results = items.map(item => {
      ...
      return {
        ...
        image, texture_url:item.texture_url || null,
        tex_x:Number(item.tex_x)||0, tex_y:Number(item.tex_y)||0,
        ...
        texture_download_url:item.texture_url || null,
        file_name:item.file_name,type:item.type,spread_type:item.spread_type,
        version:dataset.version,data_source:"items.dat + decoded textures"
      };
    });
    return json({success:true,query:q,version:dataset.version,results,source:"Growtopia decoded items.dat + decoded texture sprites + Wiki enrichment",cached_for_seconds:300});
  } catch(error) { return json({success:false,error:String(error?.message||error)},502); }
}

async function enrichWiki(items) {
  const map=new Map(); const titles=items.map(x=>x.name).filter(Boolean); if(!titles.length)return map;
  try {
    const api=new URL(WIKI_API); api.searchParams.set("action","query"); api.searchParams.set("format","json");
    api.searchParams.set("prop","extracts|pageimages|info"); api.searchParams.set("explaintext","1"); api.searchParams.set("exintro","1");
    api.searchParams.set("exchars","700"); api.searchParams.set("piprop","thumbnail"); api.searchParams.set("pithumbsize","256");
    api.searchParams.set("inprop","url"); api.searchParams.set("titles",titles.join("|")); api.searchParams.set("redirects","1");
    const r=await fetch(api,{headers:{"User-Agent":"BILSX-Growtopia-API/5.0"},cf:{cacheTtl:900,cacheEverything:true}}); if(!r.ok)return map;
    const data=await r.json(); for(const x of Object.values(data.query?.pages||{})){if(!x.title)continue;map.set(String(x.title).toLowerCase(),{description:String(x.extract||"").trim(),image:x.thumbnail?.source||null,source_url:x.fullurl||null});}
  } catch(_){} return map;
}
