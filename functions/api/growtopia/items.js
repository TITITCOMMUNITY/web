...
export async function onRequestGet({ request }) {
  ...
  try {
    ...
    api.searchParams.set("piprop","thumbnail"); api.searchParams.set("pithumbsize","160"); api.searchParams.set("inprop","url"); api.searchParams.set("format","json"); api.searchParams.set("origin","*");
    const response = await fetch(api,{headers:{"User-Agent":"BILSX-Growtopia-API/2.0"},cf:{cacheTtl:300,cacheEverything:true}});
    if(!response.ok)return json({success:false,error:`WIKI_HTTP_${response.status}`},502);
    const data=await response.json();
    const results=Object.values(data.query?.pages||{}).sort((a,b)=>(a.index||0)-(b.index||0)).map(x=>({title:x.title,page_id:x.pageid,description:String(x.extract||"").trim(),image:x.thumbnail?.source||null,source_url:x.fullurl||null}));
    return json({success:true,query:q,results,source:"Growtopia Wiki data API"});
