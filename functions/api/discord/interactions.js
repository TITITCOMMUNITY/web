...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (body.type !== 2) return json({ type: 4, data: { content: "Unsupported interaction." } });

    const discordUser = body.member?.user || body.user;
    const discordId = String(discordUser?.id || "");
    if (!discordId) return interaction("Unable to identify Discord user.", true);

    const operator = await getOperator(env.DB, discordId);
    const command = String(body.data?.name || "");

    if (!operator) {
      return interaction("⛔ You are not authorized to use BILSX management commands.", true);
    }

    if (!hasPermission(operator.permission, command)) {
      return interaction("⛔ You do not have permission to use this command.", true);
    }

    switch (command) {
      case "user": return await commandUser(env.DB, body, operator);
      case "register": return await commandRegister(env.DB, body, operator);
...
async function getOperator(db, discordId) {
  ...
  if (row) return row;
  if (discordId && discordId === String(OWNER_ID || "")) {
...
async function commandUser(db, body) {
  ...
  const remaining = active ? formatDuration(expiry - now) : "Inactive";
